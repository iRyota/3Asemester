...
int main( int argc, char* argv[] ){
  ...
  char* intCalibName = "intrinsic_param_ref.txt";
  char* extCalibName = "extrinsic_param.txt";
  char* boardFileName = "board.jpg";
  ...
  sprintf(msg1, "%lf mm %lf mm", objectPoints[4].x, objectPoints[4].y);
  cvPutText( output, msg1, cvPointFrom32f( projectedPoints[4]), 
    &font, CV_RGB(255, 0, 0));
  sprintf(msg2, "%lf mm %lf mm", objectPoints[5].x, objectPoints[5].y);
  cvPutText( output, msg2, cvPointFrom32f( projectedPoints[5]), 
    &font, CV_RGB(255, 0, 0));
...
void project( CvPoint3D32f* objectPoint, CvPoint2D32f* projectedPoint, 
  ...
  tmp[1] = R[1][0] * x[0] + R[1][1] * x[1] + R[1][2] * x[2] + o[1];
  tmp[2] = R[2][0] * x[0] + R[2][1] * x[1] + R[2][2] * x[2] + o[2];

  // compute projected~ = A * tmp;
  projected[0] = A[0][0] * tmp[0] + A[0][1] * tmp[1] + A[0][2] * tmp[2];
  projected[1] = A[1][0] * tmp[0] + A[1][1] * tmp[1] + A[1][2] * tmp[2];
  projected[2] = A[2][0] * tmp[0] + A[2][1] * tmp[1] + A[2][2] * tmp[2];
