...
void init(char *filename)
{
   ...
   // makeCheckImages();
   if((image = readJPEGimage(filename, &imageWidth, &imageHeight, &imageDepth)) == NULL){
   ...
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                   GL_NEAREST);
   glTexImage2D(GL_TEXTURE_2D, 0, imageFormat, imageWidth,
                imageHeight, 0, imageFormat, GL_UNSIGNED_BYTE,
                image);
}
