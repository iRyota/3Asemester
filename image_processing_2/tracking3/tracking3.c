...
void track_template(const CvArr* template, const CvArr* current,
    CvPoint2D32f template_feature, CvPoint2D32f *current_feature_p,
    unsigned int *ssd_array, unsigned int *maximum_p, unsigned int *minimum_p,
  ...
  for (;;) {
    ...
    if (mode == SEARCHING) {
      ...
      cvRectangle(output, cvPoint(p1x, p1y), cvPoint(p2x, p2y),
          ...
          0);
      track_template(template_gray, gray, template_point, &current_point, ssds,
      ...
      if (minimum_value
          < (THRESHOLD_BASE * template_width * template_height
              / (win_step * win_step))) {
        cvRectangle(output, cvPoint(p1x, p1y), cvPoint(p2x, p2y),
        ...
        for (i = 0; i < 16; i++) {
          if (maximum_value != minimum_value) {
            cvRectangle(
                ...
                CV_RGB(
                    0,
                    255 * (ssds[i + 16 * j] - minimum_value)
                        / (maximum_value - minimum_value), 0), -1, // thickness, -1=filled
                8, // line_type, 8=8-connected
                0);
          }
        }
      ...
      ssds = (unsigned int *) malloc(16 * 16 * sizeof(unsigned int));
...
void track_template(const CvArr* template, const CvArr* current,
  ...
  cvGetImageRawData(template, &dataR, &stepR, &sizeR);
  cvGetImageRawData(current, &dataS, &stepS, &sizeS);

  max_sum = 0;
  min_sum = 255 * 255 * template_size.height * template_size.width
      / (step * step) + 1;
  min_u = 8;
  min_v = 8;
    ...
    for (u = 0; u < 16; u++) {
      ...
      for (vv = 0; vv < template_size.height; vv += step) {
        for (uu = 0; uu < template_size.width; uu += step) {
          ...
          s = *pS;
          if (s > r) {
            sdiff = (int) (s - r) * (int) (s - r);
          } else {
            sdiff = (int) (r - s) * (int) (r - s);
          }
          sum = sum + sdiff;
        }
      }
      ssd_array[u + 16 * v] = sum;
      if (sum < min_sum) {
        min_sum = sum;
        min_u = u;
        min_v = v;
  ...
  *minimum_p = min_sum;
  *maximum_p = max_sum;
  (*current_feature_p).x = min_u * step
      + ((int) (*current_feature_p).x - 8 * step);
  (*current_feature_p).y = min_v * step
  ...
  fprintf(
      stderr,
      "x=%lf, y=%lf, min_sum=%d, max_sum=%d, thr=%d, t_width=%d, t_height=%d, step=%d\n",
      (*current_feature_p).x,
      (*current_feature_p).y,
      min_sum,
      max_sum,
      THRESHOLD_BASE * template_size.width * template_size.height
          / (step * step), template_size.width, template_size.height, step);
}
