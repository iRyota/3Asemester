...
void track_template(const CvArr* template, const CvArr* current,
    ...
    CvSize template_size, int step, char *status_p);
  ...
  for (;;) {
    ...
    if (mode == SEARCHING) {
      ...
      cvRectangle(output, cvPoint(p1x, p1y), cvPoint(p2x, p2y),
          ...
          0);
      track_template(template_gray, gray, template_point, &current_point,
...
void track_template(const CvArr* template, const CvArr* current,
  ...
  min_sum = 255 * template_size.height * template_size.width / (step * step)
      + 1;
  min_u = (int) (*current_feature_p).x;
  min_v = (int) (*current_feature_p).y;
  *status_p = BAD;
  ...
  for (v = (int) (*current_feature_p).y - 8 * step;
    ...
    for (u = (int) (*current_feature_p).x - 8 * step;
      ...
      sum = 0;
      for (vv = -template_size.height / 2; vv < template_size.height / 2; vv +=
          step) {
        for (uu = -template_size.width / 2; uu < template_size.width / 2; uu +=
            step) {
          // r...get pixel of(temp_f.x+uu, temp_f.y+vv)
          //   from template frame
  ...
  (*current_feature_p).x = min_u;
  (*current_feature_p).y = min_v;
  ...
  fprintf(
      stderr,
      "x=%lf, y=%lf, min_sum=%d, thr=%d, t_width=%d, t_height=%d, step=%d\n",
      (*current_feature_p).x,
      (*current_feature_p).y,
      min_sum,
      THRESHOLD_BASE * template_size.width * template_size.height
