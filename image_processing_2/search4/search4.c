...
void search_template(const CvArr* template, const CvArr* curr,
...
void search_template(const CvArr* template, const CvArr* curr,
  ...
  cvGetImageRawData(curr, &dataS, &stepS, &sizeS);
    ...
    for (u = size.width / 2; u < sizeS.width - size.width / 2; u += step * 5) {
      ...
      for (vv = -size.height / 2; vv < size.height / 2; vv += step) {
        ...
        for (uu = -size.width / 2; uu < size.width / 2; uu += step) {
          ...
          if (sum > THRESHOLD_BASE * size.width * size.height / (step * step)) {
            status = -1;
            break;
          }
        }
