...
void track_template(const CvArr* template, const CvArr* current,
    ...
    unsigned int *sad_array, unsigned int *maximum_p, unsigned int *minimum_p,
  ...
  for (;;) {
    ...
    if (mode == SEARCHING) {
      ...
      cvRectangle(output, cvPoint(p1x, p1y), cvPoint(p2x, p2y),
          ...
          0);
      track_template(template_gray, gray, template_point, &current_point, sads,
      ...
      sads = (unsigned int *) malloc(16 * 16 * sizeof(unsigned int));
...
void track_template(const CvArr* template, const CvArr* current,
  ...
  max_sum = 0;
  min_sum = 255 * template_size.height * template_size.width / (step * step)
      + 1;
  min_u = 8;
  min_v = 8;
