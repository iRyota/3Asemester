...
GLfloat 	distance = DISTANCE_0, pitch = 0.0, yaw = 0.0; /* カメラの位置姿勢 */
GLint		mouse_button = -1;
GLint		mouse_x = 0, mouse_y =0;
GLint		elapsed_time; /* glutInit()後の経過時間(ms単位) */
...
IplImage 	*frame, *cvimage;	/* OpenCV画像構造体 */
CvCapture* 	capture = 0;		/* Capture構造体 */
int 		dev_index = 0;		/* カメラのインデックス */
int 		cap_width =SIZE_X, cap_height =SIZE_Y;

pthread_t	video_thread; /* スレッド記述子 */
pthread_mutex_t	video_mutex; /* ミューテックス記述子 */
int		video_active = 1;
...
void *get_videoframe(void *param)
{
	...
	pthread_mutex_lock(&video_mutex); /* 他スレッドからのデータへのアクセスをロック */
	cvCvtColor(frame, cvimage, CV_BGR2RGB);
/*	cvSmooth(frame, cvimage, CV_GAUSSIAN, 15, 15, 0, 0); */
	pthread_mutex_unlock(&video_mutex); /* ロック解除 */
    }
    pthread_exit(NULL);
}
...
{
    int         i, j;
/* 課題：テクスチャ座標の指定とポリゴンの頂点座標の指定を修正せよ */
/* TODO_s と TODO_eの間を変更する */ 
    for( j=0; j<=DIV_Y; j++ ){
        for( i=0; i<=DIV_X; i++ ){
...
void idle(void)
{
    elapsed_time = glutGet(GLUT_ELAPSED_TIME); /* 経過時間の取得 */
...
/* OpenGLコールバック関数の指定 */
    ...
    pthread_mutex_init(&video_mutex, NULL); /* ミューテックスの初期化 */
    pthread_create(&video_thread, NULL, get_videoframe, (void *)NULL);/* スレッド作成 */
    glutMainLoop();
    return 0;
}
